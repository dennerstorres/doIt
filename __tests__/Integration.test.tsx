...
describe('Integration Tests', () => {
  ...
  it('should allow editing a task', async () => {
    ...
    const addTask = root.findByType(AddTask);
    await act(async () => {
      addTask.props.onChangeText('Task to Edit');
    });
    await act(async () => {
    ...
    let taskList = root.findByType(TaskList);
    const taskToEdit = taskList.props.tasks[0];

    // Edit task
    await act(async () => {
      taskList.props.handleEditTask(
        taskToEdit.id,
        'Edited Task Name',
        'high',
        'personal',
    ...
    // Verify update
    taskList = root.findByType(TaskList);
    expect(taskList.props.tasks[0].task).toBe('Edited Task Name');
    expect(taskList.props.tasks[0].priority).toBe('high');
    expect(taskList.props.tasks[0].category).toBe('personal');
    expect(taskList.props.tasks[0].repeat).toBe('daily');
  });
